  ...
  <div class="container mt-5">
    ...
    <form @submit.prevent="soumettreFormulaire" novalidate>
      ...
      <div class="mb-3">
        ...
        <div v-if="erreurs.role" class="text-danger">{{ erreurs.role }}</div>
      </div>
...
const soumettreFormulaire = async () => {
  ...
  if (!prenom.value) erreurs.value.prenom = 'Le prénom est requis.'
    ...
    erreurs.value.prenom = 'Le prénom ne peut pas dépasser 50 caractères'
  ...
  //Validation du rôle
  if (!role.value) erreurs.value.role = 'Le rôle est requis.'

  //Validation du mot de passe
  if (!motdepasse.value) erreurs.value.motdepasse = 'Mot de passe requis.'
  else if (motdepasse.value.length < 6)
    erreurs.value.motdepasse = 'Le mot de passe doit contenir au moins 6 caractères'

  //Validation de la confirmation du mot de passe
  if (!confirmation.value)
    erreurs.value.confirmation = 'Une confirmation de mot de passe est requise.'
